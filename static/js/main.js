(function ($) {
    ...
    /*$("#navLogoButton").click(function() {
      if ($("#myTopnavtopnav").hasClass("topnav")) {
    ...
    $('.qtybtn').on('click', function () {
        ...
        if ($button.hasClass('inc')) {
            // console.log('The response is: ', $(this).parattr("data").val());
            // $(this).attr("href").val("{% url 'increase' pk=cart_item.pk %}")
            // window.location.href = "{% url 'decrease' pk=cart_item.item.item.pk %}"
            // $button.attr("href", "{% url 'increase' pk=cart_item.item.pk %}");
            var newVal = parseFloat(oldValue) + 1;
        } else {
            // Don't allow decrementing below zero
            if (oldValue > 0) {
                // $(this).attr("href").val("{% url 'decrease' pk=cart_item.pk %}")
                // window.location.href = "{% url 'decrease' pk=cart_item.item.pk %}"
                // $button.attr("href", "{% url 'decrease' pk=cart_item.item.pk %}");
                var newVal = parseFloat(oldValue) - 1;
            } else {
                newVal = 0;
            }
    ...
    const showPasswordToggle = document.getElementById('eye')
    const handleTogglerInput = (e) => {
    ...
    showPasswordToggle.addEventListener("click", handleTogglerInput);




    /*function show() {
        var p = document.getElementById('c-password');
